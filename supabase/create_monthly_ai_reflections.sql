-- monthly_ai_reflections 테이블 생성 및 RLS 정책 설정
...
-- 1. 테이블 생성
CREATE TABLE IF NOT EXISTS public.monthly_ai_reflections (
...
-- 2. 인덱스 생성
CREATE INDEX IF NOT EXISTS idx_monthly_ai_reflections_user_month 
  ON public.monthly_ai_reflections(user_id, month_start);

-- 3. updated_at 트리거 설정 (기존 함수 사용)
CREATE TRIGGER set_updated_at_monthly_ai_reflections
  BEFORE UPDATE ON public.monthly_ai_reflections
  FOR EACH ROW
  EXECUTE FUNCTION public.set_updated_at();

-- 4. RLS 활성화
ALTER TABLE public.monthly_ai_reflections ENABLE ROW LEVEL SECURITY;
...
-- DROP POLICY IF EXISTS "monthly_ai_reflections_delete_own_approved" ON public.monthly_ai_reflections;
-- CREATE POLICY "monthly_ai_reflections_delete_own_approved" ON public.monthly_ai_reflections
--   FOR DELETE
--   USING (
--     user_id = auth.uid()
...
WHERE table_schema = 'public'
  AND table_name = 'monthly_ai_reflections'
